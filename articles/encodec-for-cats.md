---
title: "猫でもわかるEnCodec ― 音声を「離散トークン」に変えるニューラルコーデック"
emoji: "🗜️"
type: "tech"
topics: ["tts", "音声合成", "codec", "機械学習", "深層学習"]
published: false
---

## この記事について

[猫でもわかるLLM TTS](https://zenn.dev/nnn112358/articles/llm-tts-for-cats)で、「音声を離散トークンにするコーデック」が主役級の部品として出てきました。この記事はその代表格 **EnCodec**(2022, Meta)——**ニューラルコーデック**の話です。

EnCodec は、音声波形を **離散トークン(コード)** に圧縮し、そこから波形を復元するモデル。VALL-E をはじめ、LLM系TTSの「音声を言葉のように扱う」土台になっています。猫でもわかるように見ていきましょう。🗜️

:::message
EnCodec: Défossez et al., *"High Fidelity Neural Audio Compression"* (2022, [arXiv:2210.13438](https://arxiv.org/abs/2210.13438))。元をたどれば [VQ-VAE](https://zenn.dev/nnn112358/articles/vae-for-cats) → SoundStream の系譜。本記事の仕様は論文本文で確認しています。図は matplotlib と mermaid で作成しました。
:::
...
学習は、波形の**再構成損失**(時間・周波数の両方)＋ [GAN](https://zenn.dev/nnn112358/articles/gan-for-cats)由来の**多解像度識別器**による知覚的な損失で行います([→HiFi-GANのMSD/MRDと同じ発想](https://zenn.dev/nnn112358/articles/hifigan-for-cats))。損失の重みを自動調整する **loss balancer** も工夫の一つ。

## 心臓部:RVQ(残差ベクトル量子化)

「連続な潜在ベクトルを、離散トークンにする」のが量子化です。素朴には、**コードブック(見本ベクトルの辞書)から一番近いものを選ぶ**([→VQ-VAE](https://zenn.dev/nnn112358/articles/vae-for-cats))。でも1つの辞書で高品質を狙うと、辞書が巨大になりすぎます。
...
EnCodec が TTS で決定的に重要なのは、**音声を「離散トークンの列」に変える**からです。これで音声が、テキストの単語列と同じように扱えるようになり、**[LLMで音声を自己回帰生成する](https://zenn.dev/nnn112358/articles/llm-tts-for-cats)** 道が開けました。

- **VALL-E**:EnCodec のトークンを [LLM](https://zenn.dev/nnn112358/articles/llm-tts-for-cats) で生成 → 3秒 zero-shot クローン。
...
- TTSでは**音声を"言葉のように"扱う鍵**。VALL-E など [LLM TTS](https://zenn.dev/nnn112358/articles/llm-tts-for-cats) の土台。VQ-VAE→SoundStream→EnCodec→DACの系譜。
...
- 関連記事: [猫でもわかるLLM TTS](https://zenn.dev/nnn112358/articles/llm-tts-for-cats) / [猫でもわかるVAE(VQ-VAE)](https://zenn.dev/nnn112358/articles/vae-for-cats) / [猫でもわかるGAN](https://zenn.dev/nnn112358/articles/gan-for-cats) / [VITSから見るTTS 10系統マップ](https://zenn.dev/nnn112358/articles/tts-lineage-map-from-vits)

:::message
🐾 **猫でもわかるTTSシリーズ**(全28本) ― [目次](https://zenn.dev/nnn112358/articles/tts-for-cats-index) ／ 前: [LLM TTS](https://zenn.dev/nnn112358/articles/llm-tts-for-cats) ／ 次: [Flow Matching](https://zenn.dev/nnn112358/articles/flow-matching-for-cats)
:::
