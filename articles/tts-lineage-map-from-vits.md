...
flowchart LR
    T1("Tacotron (2017)<br/>自己回帰 mel"):::gray
    T2("Tacotron 2 (2017)<br/>+WaveNet"):::blue
    TF("Transformer TTS (2018)"):::blue
    FS("FastSpeech (2019)<br/>非AR / 並列"):::blue
    FS2("FastSpeech 2 (2020)<br/>+音高・エネルギー"):::blue
    FP("FastPitch (2020)"):::blue
    GT("Glow-TTS (2020)<br/>Flow + MAS"):::purple
    VITS("VITS (2021)<br/>単段E2E"):::amber
    T1 --> T2
    T1 --> TF
    T2 --> FS
    TF --> FS
...
%%{init: {'theme':'base','themeVariables':{'lineColor':'#475569','fontFamily':'Noto Sans CJK JP, sans-serif','fontSize':'15px'},'flowchart':{'padding':14,'nodeSpacing':50,'rankSpacing':60,'curve':'linear'}}}%%
flowchart LR
    VITS("VITS (2021)"):::amber
    VITS2("VITS2 (2023)<br/>SDP + discriminator"):::blue
    YT("YourTTS (2022)<br/>zero-shot"):::blue
    SOVITS("so-vits-svc (2023)<br/>SoftVC content encoder"):::blue
    MBI("MB-iSTFT-VITS<br/>iSTFT軽量化"):::blue
    PIPER("Piper<br/>ONNX軽量 / エッジ"):::blue
    BV("Bert-VITS2<br/>多言語BERT"):::blue
    MELO("MeloTTS<br/>VITS+VITS2+BertVITS2 統合"):::blue
    SBV("Style-Bert-VITS2<br/>JP-Extra + スタイル"):::blue
    FISH("Fish-Speech (2024)<br/>LLM + Firefly-GAN へ転換"):::amber
    OV("OpenVoice<br/>MeloTTS基盤 + クローン"):::blue
    RVC("RVC<br/>Retrieval VC"):::blue
    GPTSO("GPT-SoVITS (2024)<br/>GPT text→semantic"):::blue
    VITS --> VITS2
    VITS --> YT
    VITS --> SOVITS
    VITS --> MBI
...
%%{init: {'theme':'base','themeVariables':{'lineColor':'#475569','fontFamily':'Noto Sans CJK JP, sans-serif','fontSize':'15px'},'flowchart':{'padding':14,'nodeSpacing':50,'rankSpacing':60,'curve':'linear'}}}%%
flowchart LR
    WN("WaveNet (2016)<br/>基礎"):::gray
    WR("WaveRNN (2018)"):::blue
    LPC("LPCNet (2019)<br/>WaveRNN+線形予測<br/>CPUリアルタイム"):::blue
    FWG("FWGAN (2023)<br/>Framewise WaveGAN"):::blue
    FAR("FARGAN (2024)<br/>低複雑度 / Opusに実装"):::amber
    HIFI("HiFi-GAN (2020)<br/>VITS標準ボコーダ"):::blue
    BIG("BigVGAN (2022)"):::blue
    VOCOS("Vocos (2023)<br/>iSTFTベース"):::blue
    PWG("ParallelWaveGAN (2020)"):::blue
    WN --> WR
    WR --> LPC
    LPC --> FWG
    FWG --> FAR
...
%%{init: {'theme':'base','themeVariables':{'lineColor':'#475569','fontFamily':'Noto Sans CJK JP, sans-serif','fontSize':'15px'},'flowchart':{'padding':14,'nodeSpacing':50,'rankSpacing':60,'curve':'linear'}}}%%
flowchart LR
    VQ("VQ-VAE (2017)<br/>基礎"):::gray
    SS("SoundStream (2021)<br/>RVQ codec 発祥"):::amber
    AL("AudioLM (2022)<br/>codecトークンでLM"):::blue
    EN("EnCodec (2022, Meta)"):::blue
    DAC("DAC (2023, Descript)"):::blue
    FAC("FACodec<br/>属性分離 (NS3内)"):::blue
    DACVAE("DACVAE<br/>連続潜在化"):::blue
    VQ --> SS
    SS --> AL
    SS --> EN
    SS --> DAC
...
%%{init: {'theme':'base','themeVariables':{'lineColor':'#475569','fontFamily':'Noto Sans CJK JP, sans-serif','fontSize':'15px'},'flowchart':{'padding':14,'nodeSpacing':50,'rankSpacing':60,'curve':'linear'}}}%%
flowchart LR
    AL("AudioLM (2022)"):::gray
    VALLE("VALL-E (2023)<br/>EnCodec + AR/NAR"):::blue
    VALLE2("VALL-E 2 (2024)<br/>人間並み"):::blue
    VC("VoiceCraft<br/>遅延パターンAR"):::blue
    BARK("Bark (2023, Suno)"):::blue
    TOR("Tortoise (2022)<br/>AR + diffusion decoder"):::gray
    XTTS("XTTS / XTTS-v2 (Coqui)<br/>多言語"):::blue
    IDX("IndexTTS (2025, bilibili)<br/>産業レベル"):::blue
    AL --> VALLE
    VALLE --> VALLE2
    VALLE --> VC
    AL --> BARK
...
%%{init: {'theme':'base','themeVariables':{'lineColor':'#475569','fontFamily':'Noto Sans CJK JP, sans-serif','fontSize':'15px'},'flowchart':{'padding':14,'nodeSpacing':50,'rankSpacing':60,'curve':'linear'}}}%%
flowchart LR
    DDPM("DDPM (2020)<br/>基礎"):::gray
    GRAD("Grad-TTS (2021)<br/>score-based mel"):::blue
    MATCHA("Matcha-TTS (2024)<br/>conditional flow matching"):::blue
    FM("Flow Matching<br/>(2022, Lipman) 基礎"):::gray
    VB("Voicebox (2023, Meta)<br/>flow matching + in-context"):::blue
    E2("E2-TTS (2024, MS)<br/>filler token簡素化"):::blue
    F5("F5-TTS (2024)<br/>DiT + ConvNeXt V2"):::amber
    DIT("DiT (2023, Peebles & Xie)<br/>画像生成由来の骨格"):::purple
    DDPM --> GRAD
    GRAD --> MATCHA
    FM --> VB
    FM --> MATCHA
...
%%{init: {'theme':'base','themeVariables':{'lineColor':'#475569','fontFamily':'Noto Sans CJK JP, sans-serif','fontSize':'15px'},'flowchart':{'padding':14,'nodeSpacing':50,'rankSpacing':60,'curve':'linear'}}}%%
flowchart LR
    MG("MaskGIT (2022, 画像)"):::gray
    MUSE("Muse (2023, text-to-image)"):::blue
    MAGVIT("MAGVIT-v2 (2023, 動画)"):::blue
    SST("SoundStorm (2023)<br/>非AR並列生成"):::blue
    MASKGCT("MaskGCT (2024, Amphion)<br/>Masked Generative Codec Transformer"):::amber
    MG --> MUSE
    MG --> MAGVIT
    MG --> SST
    SST --> MASKGCT
...
%%{init: {'theme':'base','themeVariables':{'lineColor':'#475569','fontFamily':'Noto Sans CJK JP, sans-serif','fontSize':'15px'},'flowchart':{'padding':14,'nodeSpacing':50,'rankSpacing':60,'curve':'linear'}}}%%
flowchart LR
    WHISPER("Whisper (2022)<br/>ASR encoder"):::gray
    COSY("CosyVoice (2024, Alibaba)<br/>semantic tokens + LLM + CFM"):::blue
    COSY2("CosyVoice 2 (2024)<br/>streaming"):::blue
    COSY3("CosyVoice 3 (2025)"):::blue
    QWEN3("Qwen3 (LLM)"):::gray
    QWENTTS("Qwen3-TTS (2026)<br/>10言語ストリーミング"):::blue
    WHISPER --> COSY
    COSY --> COSY2
    COSY2 --> COSY3
    QWEN3 --> QWENTTS
