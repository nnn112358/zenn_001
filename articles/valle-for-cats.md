---
title: "猫でもわかるVALL-E ― 音声を「言語モデルで生成する」を始めたモデル"
emoji: "🔊"
type: "tech"
...
[猫でもわかるLLM TTS](https://zenn.dev/nnn112358/articles/llm-tts-for-cats) で「音声を離散トークンにして、言語モデルで生成する」というパラダイムを紹介しました。その**起点になったモデル**が **VALL-E**(2023, Microsoft）です。
...
:::message
VALL-E: Wang et al., *"Neural Codec Language Models are Zero-Shot Text to Speech Synthesizers"* (2023, [arXiv:2301.02111](https://arxiv.org/abs/2301.02111))。60K時間の LibriLight で学習、3秒プロンプトで zero-shot TTS。本記事の仕様・数値は論文本文で確認しています。図は mermaid で作成しました。
:::

## 3行で言うと

- VALL-E = **TTS を「音声トークンの言語モデリング」として解いた最初のモデル**。
- [EnCodec](https://zenn.dev/nnn112358/articles/encodec-for-cats) の8層 RVQ トークンを、**AR(第1層)+ NAR(第2〜8層)** の2段 Transformer で生成。
...
1. 音声を **[EnCodec](https://zenn.dev/nnn112358/articles/encodec-for-cats)** で**離散トークンの行列**にする(75Hz × 8コードブック = 10秒で 750×8 トークン)。
...
[LLM TTS の記事](https://zenn.dev/nnn112358/articles/llm-tts-for-cats)で紹介した **AR+NAR 方式**の原点です。
...
第2〜8層は**音質の細部**(テクスチャ・明瞭さ）を補います。RVQ の構造上、上位層ほど粗く、下位層ほど細かい残差を表すからです([→EnCodec の記事](https://zenn.dev/nnn112358/articles/encodec-for-cats))。
...
大量のデータで学習することで、**プロンプトの話者特徴を in-context で捉え、指定テキストをその声で生成**する能力が自然に身につきます。これが [zero-shot TTS](https://zenn.dev/nnn112358/articles/zero-shot-for-cats) の始まりです。
...
- **SoundStorm / MaskGCT**: マスク予測方式（[→LLM TTS 記事](https://zenn.dev/nnn112358/articles/llm-tts-for-cats)参照）
- **[Fish-Speech](https://zenn.dev/nnn112358/articles/fish-speech-for-cats)**: DualAR + GFSQ（RVQ 自体を捨てた）
- **[Qwen3-TTS](https://zenn.dev/nnn112358/articles/qwen-tts-for-cats)**: Multi-Token Prediction + LLM 統合
...
VALL-E は「メル回帰 → トークン言語モデル」というパラダイムシフトを起こし、[LLM TTS](https://zenn.dev/nnn112358/articles/llm-tts-for-cats) 全体の土台を築きました。
...
- 関連記事: [猫でもわかるLLM TTS](https://zenn.dev/nnn112358/articles/llm-tts-for-cats) / [猫でもわかるEnCodec](https://zenn.dev/nnn112358/articles/encodec-for-cats) / [猫でもわかるzero-shot TTS](https://zenn.dev/nnn112358/articles/zero-shot-for-cats) / [猫でもわかるFish-Speech](https://zenn.dev/nnn112358/articles/fish-speech-for-cats) / [猫でもわかるQwen3-TTS](https://zenn.dev/nnn112358/articles/qwen-tts-for-cats) / [VITSから見るTTS 10系統マップ](https://zenn.dev/nnn112358/articles/tts-lineage-map-from-vits)

:::message
🐾 **猫でもわかるTTSシリーズ**(全32本) ― [目次](https://zenn.dev/nnn112358/articles/tts-for-cats-index) ／ 前: [FastSpeech](https://zenn.dev/nnn112358/articles/fastspeech-for-cats)
:::
