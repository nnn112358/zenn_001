...
```mermaid
graph LR
    T["テキスト"] --> AM["音響モデル<br/>(Tacotron2 / FastSpeech2)"]
    AM -->|"メルスペクトログラム"| VO["ボコーダ<br/>★HiFi-GAN★"]
    VO --> W["波形(音)"]
```
...
```mermaid
graph LR
    M["mel<br/>80 × T"] --> U1["転置畳み込み<br/>×2, ×2, ..."]
    U1 --> MRF["MRF<br/>いろんな受容野で<br/>パターンを観察"]
    MRF --> W["波形<br/>1 × 256T"]
```
...
```mermaid
graph TD
    MEL["メルスペクトログラム"] --> G["Generator<br/>(偽造者)"]
    G --> FAKE["生成した波形<br/>(ニセモノ)"]
    REAL["本物の波形"] --> D["MPD + MSD<br/>(2人の鑑定士)"]
    FAKE --> D
    D --> LOSS["損失<br/>敵対的 + メル + 特徴マッチング"]
    LOSS -.->|"更新"| G
    LOSS -.->|"更新"| D
```
