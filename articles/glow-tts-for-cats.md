---
title: "猫でもわかるGlow-TTS ― Flow と MAS が出会う、VITSの前身"
emoji: "✨"
type: "tech"
...
これまで [Flow(正規化フロー)](https://zenn.dev/nnn112358/articles/flow-for-cats) と [MAS(単調アライメント探索)](https://zenn.dev/nnn112358/articles/mas-for-cats) を別々に解説してきました。**Glow-TTS** は、その2つが**出会って1つの音響モデルになる場所**です。そして [VITS](https://zenn.dev/nnn112358/articles/vits-for-cats) の**直接の前身**でもあります。
...
Glow-TTS は [音響モデル](https://zenn.dev/nnn112358/articles/acoustic-model-for-cats)、つまり**テキスト(音素)→ メルスペクトログラム**を担う部分です。従来主流だった Tacotron 2 は自己回帰＋注意で、自然な反面、**1フレームずつなので遅く**、**注意が壊れると単語のスキップ・繰り返し**が起きました。
...
Glow-TTS の背骨は [正規化フロー](https://zenn.dev/nnn112358/articles/flow-for-cats)です。メルの条件付き分布 `P(mel | テキスト)` を、単純な事前分布 `P(z | テキスト)` を**可逆な変換 f で写して**表現します。学習時はメルを `z` に変換して(順方向)、変数変換の公式で**厳密な尤度**を計算し最大化。生成時は逆に `z` からメルを作ります(逆方向)。

デコーダの中身は、[Flow の記事](https://zenn.dev/nnn112358/articles/flow-for-cats)で出てきた **ActNorm + 可逆1×1畳み込み + アフィンカップリング** のブロックの積み重ね(WaveGlow/Glow と同じ部品)。可逆なので、順・逆どちらも**並列**に計算でき、これが高速生成につながります。

## 柱②:MAS(単調アライメント探索)

もう一つの柱が [MAS](https://zenn.dev/nnn112358/articles/mas-for-cats)。実は **MAS はこの Glow-TTS で提案された**アルゴリズムです。

テキスト側は、各音素について「その音素らしい潜在の事前分布(平均 μ)」を持っています。MAS は、音声から得た `z` に対して、**尤度が最大になる単調な対応**を動的計画法で探します。外部ツールに頼らず、しかも単調(順序を守り飛ばさない)なので、注意のような崩壊が起きません。詳しくは[MASの記事](https://zenn.dev/nnn112358/articles/mas-for-cats)へ。
...
学習時は逆に、正解メルを Flow で `z` に変換し、MAS でアライメントを求め、そのアライメントから得た継続長を**継続長予測器の教師**にします([→この継続長がのちに VITS では SDP の教師になる](https://zenn.dev/nnn112358/articles/sdp-for-cats))。
...
継続長を変えれば**話速**を、事前分布のばらつき(温度)を変えれば**声のゆらぎ**を調整できるのも、Glow-TTS の利点です。なお Glow-TTS が出すのはメルまでなので、波形にするには [HiFi-GAN](https://zenn.dev/nnn112358/articles/hifigan-for-cats) などの**ボコーダが別途必要**(2段構成)です。

## そして VITS へ

Glow-TTS の Flow + MAS は、そのまま [VITS](https://zenn.dev/nnn112358/articles/vits-for-cats) に受け継がれました。VITS は Glow-TTS に、

- **VAE**([→V](https://zenn.dev/nnn112358/articles/vae-for-cats))で音声側と潜在をつなぎ、メルを中間に出さない**単段E2E**化、
- **GAN**([→HiFi-GAN generator](https://zenn.dev/nnn112358/articles/hifigan-for-cats))を波形デコーダに、
- 継続長を決め打ちから**確率的([→SDP](https://zenn.dev/nnn112358/articles/sdp-for-cats))** に、
...
これまで別々に見てきた [Flow](https://zenn.dev/nnn112358/articles/flow-for-cats) と [MAS](https://zenn.dev/nnn112358/articles/mas-for-cats) が、Glow-TTS で1つの形になり、[VITS](https://zenn.dev/nnn112358/articles/vits-for-cats) へと続いていく——シリーズの点と点がつながる記事でした。
...
:::message
🐾 **猫でもわかるTTSシリーズ**(全28本) ― [目次](https://zenn.dev/nnn112358/articles/tts-for-cats-index) ／ 前: [SDP](https://zenn.dev/nnn112358/articles/sdp-for-cats) ／ 次: [VITS](https://zenn.dev/nnn112358/articles/vits-for-cats)
:::
