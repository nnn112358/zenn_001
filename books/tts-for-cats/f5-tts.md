...
[Flow Matching](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/flow-matching)の応用として、いま人気の **F5-TTS**(2024)を見ます。名前は "**F**airytaler that **F**akes **F**luent and **F**aithful speech with **F**low matching"(Fが5つ)。
...
- F5-TTS = **[Flow Matching](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/flow-matching) + DiT(Diffusion Transformer)** の完全非自己回帰TTS。
...
これまで見た音響モデル([→章](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/acoustic-model))は、「各音素が何フレーム続くか」を決める **継続長予測器** や、テキストと音の対応をとる **アライメント([MAS](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/mas)等)** が必須でした。

F5-TTS(と、その元になった **E2 TTS**)は、これを大胆に捨てます。やり方はこう。**文字列を、フィラー(埋め草)トークンで音声の長さまで水増し**して、あとは **[Flow Matching](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/flow-matching) で音声を"穴埋め"** するだけ。
...
- **① ConvNeXt V2 でテキストを整える**:連結の前に、文字表現を [ConvNeXt](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/vocos) V2 ブロックで洗練。音声とそろえやすくなり、アライメントが安定。
...
- **非自己回帰**なので[並列生成で速い](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/llm-tts)(RTF 0.15)。
- **zero-shot**:参照音声をプロンプトに与えれば、その声で喋る([→zero-shot](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/zero-shot))。多言語のコードスイッチや速度制御も得意。
...
- 関連する章: [Flow Matching](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/flow-matching) / [zero-shot TTS](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/zero-shot) / [Vocos(ConvNeXt)](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/vocos) / [VITSから見るTTS 10系統マップ](https://zenn.dev/nnn112358/articles/tts-lineage-map-from-vits)
