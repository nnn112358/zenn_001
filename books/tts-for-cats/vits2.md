...
[VITS](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/vits)で、VAE + Flow + GAN + MAS + SDP を1つに束ねた単段E2E TTS を見ました。この章はその**正常進化版 VITS2**(2023)です。
...
[VITS](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/vits) は「VAE + Flow + GAN + MAS + SDP」の合わせ技で、人間並みの品質を達成しました。ただ VITS2 の論文は、VITS になお残る弱点をこう挙げています。
...
VITS の [SDP](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/sdp) は、フローベースで最尤推定的に学習していました。VITS2 はこれを **敵対的学習(GAN)** に変えます。ポイントは **time step-wise conditional discriminator(トークン単位の条件付き識別器)**。**各音素トークンの継続長を、1つずつ「本物らしいか」判定**する識別器を置き、生成器(SDP)はそれを騙すように学習します。

![VITS2:継続長予測を敵対的学習に](/images/vits2-dur-adv.png)
*SDP が各音素の継続長を予測 → 識別器が各トークンごとに「本物の継続長らしいか」を判定([→敵対的学習](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/gan)) → 敵対的に自然なリズムへ。VITSより学習が効率的で、品質も向上する。*

これにより、継続長予測の**学習効率と品質**が上がりました。

## 改良② 正規化フローに Transformer ブロック

VITS の[正規化フロー](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/flow)は、アフィンカップリング(＋畳み込み)が中心で、**遠く離れた位置どうしの関係(長距離依存)**を捉えるのは得意ではありませんでした。
...
- **MAS にガウスノイズ**:VITS2 は [MAS](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/mas) にノイズを加える修正版を使い、アライメント探索を改善します。
...
- 関連する章: [VITS](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/vits) / [SDP](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/sdp) / [GAN](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/gan) / [Flow](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/flow) / [VITSから見るTTS 10系統マップ](https://zenn.dev/nnn112358/articles/tts-lineage-map-from-vits)
