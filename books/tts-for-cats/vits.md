...
テキスト(音素列)は短く、潜在 z は長い。この**対応づけ(アライメント)**を、VITSは外部アライナー無しで解きます。使うのは[音響モデルの記事](https://zenn.dev/nnn112358/books/tts-for-cats/viewer/acoustic-model)で触れた **MAS(Monotonic Alignment Search)**(Glow-TTS 由来)。
