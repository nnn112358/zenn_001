...
ここまでの章では、メルスペクトログラムを**回帰(直接予測)**するか、**Flow(可逆変換)**で生成するモデルを見てきました。今回は第3の生成アプローチ——**拡散モデル(Diffusion Model)**をTTSに持ち込んだ **Grad-TTS**(2021, Huawei Noah's Ark Lab)を見ます。
...
:::message
Grad-TTS: Popov et al., *"Grad-TTS: A Diffusion Probabilistic Model for Text-to-Speech"* (2021, ICML, [arXiv:2105.06337](https://arxiv.org/abs/2105.06337))。LJSpeech(24時間)で学習、MOS 4.44(GT 4.53)。15Mパラメータ。本章の仕様・数値は論文本文で確認しています。図は mermaid で作成しました。
:::
...
エンコーダは [Glow-TTS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/glow-tts) と同一構造(3層 Conv pre-net + Transformer 6層)。アライメントは **[MAS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/mas)** で教師なし学習。継続長予測器も同じ 2層 Conv 構成。合計 **7.2M パラメータ**。
...
Grad-TTS と [Glow-TTS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/glow-tts) はエンコーダと MAS を共有していますが、デコーダの生成戦略が対照的です。
...
Grad-TTS は **拡散モデル系TTS の起点**です。後継の DiffSinger は「浅い拡散」(エンコーダ出力から途中のステップで開始)で推論を高速化。NaturalSpeech 2/3 は拡散をコーデックトークンの潜在空間で行い、さらにスケール。一方 [Flow Matching](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/flow-matching) は拡散の「曲がった経路」を直線化し、[F5-TTS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/f5-tts) のような簡潔なモデルを生みました。
...
- 関連する章: [Glow-TTS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/glow-tts) / [MAS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/mas) / [Flow Matching](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/flow-matching) / [F5-TTS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/f5-tts) / [VITS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/vits)
