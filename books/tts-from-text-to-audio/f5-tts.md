---
title: "F5-TTS ― 継続長予測もテキストエンコーダも無しで喋る"
---

## この章について

[Flow Matching](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/flow-matching)の応用として、いま人気の **F5-TTS**(2024)を見ます。名前は "**F**airytaler that **F**akes **F**luent and **F**aithful speech with **F**low matching"(Fが5つ)。

F5-TTS のすごさは、**継続長予測器も、テキストエンコーダも、音素アライメントも要らない**という潔さ。それでいて高品質な zero-shot 音声合成ができ、しかも速い(RTF 0.15)。その仕掛けを見ていきましょう。🧚

:::message
F5-TTS: Chen et al. (2024, [arXiv:2410.06885](https://arxiv.org/abs/2410.06885))。公開100K時間・多言語で学習。本章の仕様は論文本文で確認しています。図は matplotlib と mermaid で作成しました。
:::

## 3行で言うと

- F5-TTS = **[Flow Matching](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/flow-matching) + DiT(Diffusion Transformer)** の完全非自己回帰TTS。
...
これまで見た音響モデル([→章](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/acoustic-model))は、「各音素が何フレーム続くか」を決める **継続長予測器** や、テキストと音の対応をとる **アライメント([MAS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/mas)等)** が必須でした。

F5-TTS(と、その元になった **E2 TTS**)は、これを大胆に捨てます。やり方はこう。**文字列を、フィラー(埋め草)トークンで音声の長さまで水増し**して、あとは **[Flow Matching](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/flow-matching) で音声を"穴埋め"** するだけ。
...
- **① ConvNeXt V2 でテキストを整える**:連結の前に、文字表現を [ConvNeXt](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/vocos) V2 ブロックで洗練。音声とそろえやすくなり、アライメントが安定。
...
- **非自己回帰**なので[並列生成で速い](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/llm-tts)(RTF 0.15)。
- **zero-shot**:参照音声をプロンプトに与えれば、その声で喋る([→zero-shot](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/zero-shot))。多言語のコードスイッチや速度制御も得意。
...
- 関連する章: [Flow Matching](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/flow-matching) / [zero-shot TTS](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/zero-shot) / [Vocos(ConvNeXt)](https://zenn.dev/nnn112358/books/tts-from-text-to-audio/viewer/vocos) / [VITSから見るTTS 10系統マップ](https://zenn.dev/nnn112358/articles/tts-lineage-map-from-vits)
